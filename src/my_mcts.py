...
TOTAL_CHOICES = [1, -1, 2, -2]
AVAILABLE_CHOICE_NUMBER = len(TOTAL_CHOICES)
MAX_ROUND_NUMBER = 10
# 0 < PT <= 1
PT = 0.9
CURRENT_MAX_QS = 0
...
# TODO 有expanded的和unexpanded的,且全局共享
...
class State(object):
  ...
  """
    ...
    def reach_terminal_number(self):
        # The round index starts from 1 to max round number
        # TODO 这里还有另外两种情况：current_value > PT 或者 all_expand（因为树节点具有不重复性）.后者要在node处调用
        return self.current_round_index == MAX_ROUND_NUMBER

    def get_next_state_with_random_choice(self):
        # TODO 这里改为接一个剩下的PS最大的Set
        random_choice = random.choice([choice for choice in TOTAL_CHOICES])
...
class Node(object):
  ...
  """
    ...
    def update_quality_value_if_greater(self, value):
        if value > self.quality_value:
    ...
    def __repr__(self):
        return "Node_Hash_Number: {}, Q_Val: {}/{}, state: {}".format(
...
def tree_policy(node):
  ...
  """
    ...
    while not node.get_state().reach_terminal_number():
...
def default_policy(node):
  ...
  """
    ...
    while not current_state.reach_terminal_number():
...
def monte_carlo_tree_search(node):
  ...
  蒙特卡洛树搜索包含四个步骤，Selection、Expansion、Simulation、Back_propagation。
